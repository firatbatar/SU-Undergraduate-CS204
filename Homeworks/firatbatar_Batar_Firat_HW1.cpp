...
bool checkWord(vector<vector<char>>& matrix, vector<int>& startPos, string word, int matrixSize, bool clockwise) {
	vector<vector<int>> neigbourDir = findNeighbourDirs(startPos[0], startPos[1], matrixSize);  // Get the available neighbours of the starting cell
	...
	for (int i = 0; i < neigbourDir.size(); i++) {
		...
		if (word[1] == neighbourChar) {  // If it matches the second letter of the word then the we can contiune to search from that direction
	...
	for (int i = 0; i < validDirections.size(); i++) {
		...
		int stepCounter = 0; int counter = 0;  // Counters for next steps
		char neighbourChar;
		while (len != goalLen) {
			nextStep(step, stepCounter, counter, clockwise);  // Get the next step (as a direction)
			currentPos = { currentPos[0] + step[0], currentPos[1] + step[1] };  // Get the position for the neighbour (and move)
...
int main() {
	...
	if (numOfWords == -1 || numOfWords == 0) {  // Probably an extra row of matrix (or an actual problem with # of words)
		cout << "Error: Input file is not in correct format!" << endl;
		return 0;
	}
	...
	for (int i = 0; i < matrixSize; i++) {
		for (int j = 0; j < matrixSize; j++) {
			...
			for (int idx = words.size() - 1; idx >= 0; idx--) {
				if (cell == words[idx][0]) {
					vector<int> pos = { i, j };
					bool wordCheckCC = checkWord(matrix, pos, words[idx], matrixSize, false);  // Check for the word counter-clockwise
					bool wordCheckC = checkWord(matrix, pos, words[idx], matrixSize, true);  // Check for the word clockwise
